...
data_path = Path(__file__).parent / "data_sample.json"
html_path = Path(__file__).parent / "data_sample.html"
...
def render_model_results(rec):
    models = ["ds_r1", "llama3", "llama3_2"]
    tabs = []
    tab_contents = []
    pmid = rec["pubmed_data"]["pmid"]
    for i, model in enumerate(models):
        tab_id = f"tab-{model}-{pmid}"
        tabs.append(f'<button class="tablinks{" active" if i==0 else ""}" onclick="openTab(event, \'{tab_id}\')">{model}</button>')
        model_result = rec["result"][model]
